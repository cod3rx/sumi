...
#if defined(NDEBUG)
# error "Sumi cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x739413c3db83c70278a855d421485a6ad90d63772a51760ae240c9976b55ad76");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sumi: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sumi Signed Message:\n";
...
{
    // Sumi: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Sumi: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sumi: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Sumi: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sumi: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sumi: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x374efd64be4e2d4154a166a189f57e3ad2ae2a18002ad593cda786cbe9bced76");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Sandzak CORE dev TEAM";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1527657025;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 386709054;

        if (fTestNet)
        {
            block.nTime    = 1527656987;
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfb6cdf97765a08ba057c1bd3f72f88ab96d62afc58d9f9bd8d93a01b888f0dfc"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Sumi: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SumiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SumiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SumiMiner(CWallet *pwallet)
{
    printf("SumiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sumi-miner");
    ...
    try { loop {
        ...
        printf("Running SumiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SumiMiner, pwallet));
}
